...
fullsubj <- readRDS("SB2021_individualpredictions.rds") %>% filter(exp == experiment)
...
UVSDTpreds <- fullsubj %>%
  ...
  mutate(whowins = case_when(condid %in% GumbelWin ~ "Gumbel",
                             ...
                             TRUE ~ "Else")) %>%
  mutate(model = factor(model,levels = c(models,"Data"),labels = c("EVSDT","UVSDT","Gumbel","Data"))) %>%
...
partable <- bestfit %>% 
  select(model,exp,condid,id,condition,objective,convergence,muo,sigo,c1,dc1,dc2,dc3,dc4) %>%
  pivot_longer(!c(exp,condid,id,condition,model),names_to="estimates",values_to="value") %>%
  mutate(estimates = factor(estimates, levels = rev(c("objective","convergence",
                                                "muo","sigo","c1","dc1","dc2","dc3","dc4"))))
...
ind <- allfits %>% filter(condid == subjid) %>%
  ...
  mutate(model = factor(model,levels = c("A",models),labels = c("Winning","EVSDT","UVSDT","Gumbel")))
...
resids <- ggplot(allresiduals %>% filter(condid == subjid),
  ...
  scale_color_manual(name = "Fitted Model",values = c("#56B4E9","#E69F00", "#009E73")) +
  scale_fill_manual(name = "Fitted Model",values = c("#56B4E9","#E69F00", "#009E73")) +
  scale_shape_manual(name = "Fitted Model", values = c(22,21,24))+
  
  geom_line(size = 1, position = position_dodge(width = .5))+
  geom_point(size = 3, position = position_dodge(width = .5),color="black")+
    facet_grid(oldnew~.) +
...
AICplot <- ggplot(ind,
 ...
 geom_hline(yintercept = 1, linetype="dashed")+
  ...
  scale_fill_manual(name = "Fitted Model",values = c("#000000","#56B4E9","#E69F00","#009E73")) +
  coord_cartesian(ylim = c(0,15))+
  scale_y_continuous(limits = c(0,maxDelta),name = expression(paste(Delta, " AIC")),breaks=c(0,1,2,5,10,15)) +
 
 scale_x_discrete()+
  geom_text(data = ind, aes(x = 1,y = mean(maxDelta/2),
...
zroc <- ggplot(Datonly %>%  filter(condid == subjid),
  ...
  scale_shape_manual(name = "Fitted Model",values = c(16,15,17))+
...
parametertable <-  ggplot(data = partable %>% filter(condid == subjid),
  ...
  scale_x_discrete(position = "bottom",labels = rev(c("-LL","con.",expression(mu[o]),expression(sigma[o]),"c1",
                                               "dc1","dc2","dc3","dc4")))+
    scale_y_discrete(position = "right",labels = c("EVSDT","UVSDT","Gumbel"))+
    coord_flip()+
  theme(axis.title = element_blank(),
        axis.text.y = element_text(size=12),
        axis.ticks = element_blank(),
...
plots <- plot_grid(AICplot,parametertable,zroc,confrating,resids,nrow=1,labels="AUTO",
                   rel_widths=c(0.4,0.3,0.6,0.4,0.6))
...
* **B** : Parameter estimates (con. = nlminb convergence output)
...
```{r, echo=FALSE,message=FALSE,warning=FALSE,results="asis",fig.width = 18,fig.height=4}
